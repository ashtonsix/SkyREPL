...
// basic e2e-smoke.test.ts does not cover. No OrbStack required.
//
// Run:
...
describe("Idempotency deduplication", () => {
  test("second submission with same idempotencyKey returns same workflow_id", async () => {
    ...
    // First submission
    const res1 = await fetch(`${baseUrl}/v1/workflows/launch-run`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const res2 = await fetch(`${baseUrl}/v1/workflows/launch-run`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
describe("Agent heartbeat", () => {
  ...
  test("heartbeat for nonexistent instance does not error (fire-and-forget update)", async () => {
    // The heartbeat endpoint does a blind UPDATE, which is fine if no rows match
    const res = await fetch(`${baseUrl}/v1/agent/heartbeat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
describe("Agent log ingestion + storage", () => {
  ...
  test("POST /v1/agent/logs creates log blob and returns ack with bytes_received", async () => {
    ...
    const res = await fetch(`${baseUrl}/v1/agent/logs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: testRunId,
  ...
  test("subsequent log POST appends data and blob grows", async () => {
    ...
    const res2 = await fetch(`${baseUrl}/v1/agent/logs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: testRunId,
...
describe("Agent SSE command stream", () => {
  ...
  test("pending commands are flushed when agent connects via SSE", async () => {
    ...
    let sseText = "";
    // Read a couple of chunks to get both the pending command and the connection ack
    for (let i = 0; i < 3; i++) {
...
describe("Multiple WS subscribers to same run logs", () => {
  test("both subscribers receive broadcast; one closing does not affect the other", async () => {
    ...
    // Post a log message
    await fetch(`${baseUrl}/v1/agent/logs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: runId,
...
describe("SSE workflow stream replay", () => {
  ...
  test("SSE stream returns 404 for nonexistent workflow", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows/999999/stream`);
...
describe("API error handling", () => {
  ...
  test("GET on nonexistent instance returns 404", async () => {
    const res = await fetch(`${baseUrl}/v1/instances/99999`);
    expect(res.status).toBe(404);
    const body = await res.json();
    expect(body.error).toBeDefined();
  ...
  test("GET on nonexistent run returns 404", async () => {
    const res = await fetch(`${baseUrl}/v1/runs/99999`);
    expect(res.status).toBe(404);
  });

  test("GET on nonexistent workflow returns 404", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows/99999`);
    expect(res.status).toBe(404);
  });
  ...
  test("POST launch-run with command and spec but no maxDurationMs returns 400", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows/launch-run`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const body = await res.json();
    expect(body.error.code).toBe("INVALID_INPUT");
    expect(body.error.message).toContain("maxDurationMs");
  });

  test("GET /v1/workflows/not-a-number handled gracefully", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows/not-a-number`);
  ...
  test("GET /v1/blobs/999/download returns 404", async () => {
    const res = await fetch(`${baseUrl}/v1/blobs/999/download`);
    expect(res.status).toBe(404);
  });
});
...
describe("List endpoints with filters", () => {
  ...
  test("GET /v1/workflows?type=launch-run returns only launch-run workflows", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows?type=launch-run`);
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  test("GET /v1/workflows?status=failed returns only failed workflows", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows?status=failed`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.data.length).toBeGreaterThanOrEqual(1);
    for (const wf of body.data) {
  ...
  test("GET /v1/workflows?status=completed returns empty when none completed (no OrbStack)", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows?status=completed`);
    expect(res.status).toBe(200);
    const body = await res.json();
    // In test env without OrbStack, all workflows fail at spawn-instance
    // so completed should be empty (unless some test mocked completion)
    expect(Array.isArray(body.data)).toBe(true);
  });

  test("GET /v1/workflows?type=nonexistent returns empty array", async () => {
    const res = await fetch(`${baseUrl}/v1/workflows?type=nonexistent`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.data).toEqual([]);
  });

  test("GET /v1/runs returns all runs", async () => {
  ...
  test("GET /v1/instances?provider=nonexistent returns empty array", async () => {
    const res = await fetch(`${baseUrl}/v1/instances?provider=nonexistent`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.data).toEqual([]);
  });

  test("GET /v1/allocations returns array", async () => {
    const res = await fetch(`${baseUrl}/v1/allocations`);
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(Array.isArray(body.data)).toBe(true);
  });
...
describe("Agent status endpoint", () => {
  test("POST /v1/agent/status updates run workflow_state", async () => {
    const launch = await launchRun({ command: "echo status-test" });
    ...
    const run = getRun(launch.run_id);
    expect(run).toBeDefined();
    expect(run!.workflow_state).toBe("launch-run:running");
  });

  test("POST /v1/agent/status with completed sets finished_at", async () => {
    const launch = await launchRun({ command: "echo status-complete" });
    ...
    const run = getRun(launch.run_id);
    expect(run).toBeDefined();
    expect(run!.workflow_state).toBe("launch-run:complete");
...
describe("Panic diagnostics endpoint", () => {
  test("POST /v1/instances/:id/panic stores panic record", async () => {
    ...
    const res = await fetch(`${baseUrl}/v1/instances/${instance.id}/panic`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        instance_id: instance.id,
...
describe("Stub endpoints return ack", () => {
  test("POST /v1/agent/spot-interrupt-start returns ack", async () => {
    const res = await fetch(`${baseUrl}/v1/agent/spot-interrupt-start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: 1,
  ...
  test("POST /v1/agent/spot-interrupt-complete returns ack", async () => {
    const res = await fetch(`${baseUrl}/v1/agent/spot-interrupt-complete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: 1,
  ...
  test("POST /v1/agent/heartbeat-panic-start returns ack", async () => {
    const res = await fetch(`${baseUrl}/v1/agent/heartbeat-panic-start`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: 1,
  ...
  test("POST /v1/agent/heartbeat-panic-complete returns ack", async () => {
    const res = await fetch(`${baseUrl}/v1/agent/heartbeat-panic-complete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: 1,
...
describe("Agent file download", () => {
  ...
  test("GET /v1/agent/download/malicious.py returns 404", async () => {
    const res = await fetch(`${baseUrl}/v1/agent/download/malicious.py`);
    expect(res.status).toBe(404);
  });
