...
describe("BT5: findWarmAllocation scoring", () => {
  test("returns null when no AVAILABLE allocations exist", () => {
    const result = findWarmAllocation({ spec: "gpu-small" });
  ...
  test("filters by region when specified", () => {
    ...
    const result = findWarmAllocation({ spec: "gpu-small", region: "eu-west-1" });
    expect(result).not.toBeNull();
  ...
  test("scores 100 for exact init_checksum match", () => {
    ...
    const result = findWarmAllocation({ spec: "gpu-small" }, "abc123");
    expect(result).not.toBeNull();
  ...
  test("only considers instances with workflow_state ending in :complete", () => {
...
describe("Warm claim with CAS retry", () => {
  test("claimAllocation succeeds on first try for AVAILABLE allocation", () => {
    ...
    const result = claimAllocation(alloc.id, runId);
  ...
  test("claimAllocation returns WRONG_STATE for already-claimed allocation", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);
    ...
    const result1 = claimAllocation(alloc.id, runId1);
...
describe("Replenishment in finalize", () => {
  ...
  test("does NOT replenish on non-zero exit code", async () => {
    const { instanceId, ctx } = setupFinalizeForReplenishment({ exitCode: 1 });

    const { finalizeExecutor } = await import("../../control/src/workflow/nodes/finalize");
    await finalizeExecutor.execute(ctx as any);

    // Should NOT have a new AVAILABLE allocation
    const newAllocs = queryMany<Allocation>(
      "SELECT * FROM allocations WHERE instance_id = ? AND status = 'AVAILABLE'",
      [instanceId]
    );
...
describe("Background reconciliation helpers", () => {
  test("findStaleClaimed returns CLAIMED allocations older than cutoff", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);
    const runId = createTestRun();
    ...
    // Make it stale by setting updated_at in the past
    const db = getDatabase();
    db.run("UPDATE allocations SET updated_at = ? WHERE id = ?", [Date.now() - 600_000, alloc.id]);

    const stale = findStaleClaimed(Date.now() - 300_000); // cutoff = 5 min ago
    expect(stale.length).toBe(1);
    expect(stale[0].id).toBe(alloc.id);
  });

  test("findStaleClaimed does not return fresh CLAIMED allocations", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);
    const runId = createTestRun();
    claimAllocation(alloc.id, runId);
    ...
    // Not stale yet
    const stale = findStaleClaimed(Date.now() - 600_000); // cutoff = 10 min ago
    expect(stale.length).toBe(0);
  });

  test("findExpiredAvailable returns AVAILABLE allocations older than cutoff", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);

    // Make it old by setting created_at in the past
    const db = getDatabase();
    db.run("UPDATE allocations SET created_at = ? WHERE id = ?", [Date.now() - 7_200_000, alloc.id]);

    const expired = findExpiredAvailable(Date.now() - 3_600_000); // cutoff = 1 hour ago
    expect(expired.length).toBe(1);
    expect(expired[0].id).toBe(alloc.id);
  });

  test("findExpiredAvailable does not return fresh AVAILABLE allocations", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    createAvailableAllocation(instance.id);

    const expired = findExpiredAvailable(Date.now() - 3_600_000);
    expect(expired.length).toBe(0);
  });

  test("failAllocation transitions CLAIMED -> FAILED", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);
    const runId = createTestRun();
    claimAllocation(alloc.id, runId);
  ...
  test("failAllocation transitions AVAILABLE -> FAILED", () => {
    const instance = createTestInstance({ spec: "gpu-small" });
    const alloc = createAvailableAllocation(instance.id);
...
describe("Full warm pool lifecycle", () => {
  test("spawn -> active -> complete -> replenish -> find warm -> claim", () => {
    ...
    const found = findWarmAllocation({ spec: "gpu-small" }, "abc123");
