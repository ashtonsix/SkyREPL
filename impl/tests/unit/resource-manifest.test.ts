...
describe("manifest: getManifestResourceSummary", () => {
  test("returns correct counts by type", () => {
    ...
    addResourceToManifest(manifest.id, "instance", String(instance1.id));
    addResourceToManifest(manifest.id, "instance", String(instance2.id));
    addResourceToManifest(manifest.id, "object", "blob-abc");
    addResourceToManifest(manifest.id, "object", "blob-def");
    addResourceToManifest(manifest.id, "object", "blob-ghi");

    const summary = getManifestResourceSummary(manifest.id);

    expect(summary["instance"]).toBe(2);
    expect(summary["object"]).toBe(3);
  });

  test("returns empty object for manifest with no resources", () => {
    const workflow = createTestWorkflow();
    const manifest = createTestManifest(workflow.id);
  ...
  test("returns empty object for non-existent manifest", () => {
    const summary = getManifestResourceSummary(99999);
    expect(Object.keys(summary).length).toBe(0);
  });
});
