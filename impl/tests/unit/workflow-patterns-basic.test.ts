...
describe("Workflow Patterns Basic (#WF-05, #WF-06)", () => {
  ...
  describe("pause / resume (#WF-05)", () => {
    test("pauses a running workflow and resumes it", async () => {
      // Use a gate to ensure the second node waits long enough for us to pause
      ...
      registerNodeExecutor({
        ...
        async execute() {
          step2Started = true;
          // Slow enough that the workflow will be paused between step1 and step2
      ...
      // Wait a bit and confirm it stays paused (third node should NOT start)
      await Bun.sleep(800);
    ...
    test("cancel works on paused workflow", async () => {
      ...
      // Poll until workflow is running (node execution started means it should be)
      for (let i = 0; i < 50; i++) {
