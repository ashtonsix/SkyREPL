 ...
 *   1. Exit code was 0 (clean exit)
 *   2. Instance is healthy (heartbeat within stale threshold)
 *   3. Instance has fewer than MAX_ALLOCATIONS_PER_INSTANCE active allocations
 *   4. Instance was not spot-interrupted
 *
...
function tryReplenishWarmPool(ctx: NodeContext, input: FinalizeInput): boolean {
  try {
    // Only replenish on clean exit
    if (input.exitCode !== 0) {
      ctx.log("debug", "Skipping replenishment: non-zero exit code", { exitCode: input.exitCode });
      return false;
    }
